	...
	async.waterfall([
		...
		,function(callback) {
			...
			db.client.query(
				...
				+ " FROM (SELECT DISTINCT ON(LCODE)  LCODE, MEAN, TCOUNT, CITY AS DONG,  LAT, LNG, MAX(RDATE) AS RDATE "
				+ " FROM APT_STATS"
				+ " WHERE RDATE > now() - interval '3 months' and geolevel = 'D' AND RTYPE = $1 "
				+ " AND LAT > $2 AND LAT < $3 AND LNG > $4 AND LNG < $5 "
				+ " GROUP BY LCODE, MEAN, TCOUNT, DONG, LAT, LNG, RDATE) AS LATEST_STATS"
				+ " INNER JOIN"
				+ " APT_STATS ON APT_STATS.LCODE = LATEST_STATS.LCODE AND APT_STATS.RDATE = LATEST_STATS.RDATE "
				+ " WHERE RTYPE = $1;"
	...
	async.waterfall([
		...
		,function(callback) {
			...
			db.client.query(
				...
				+ " FROM (SELECT DISTINCT ON(LCODE)  LCODE, MEAN, TCOUNT, CITY AS DONG,  LAT, LNG, MAX(RDATE) AS RDATE "
				+ " FROM APT_STATS"
				+ " WHERE RDATE > now() - interval '3 months' and geolevel = 'G' AND RTYPE = $1 "
				+ " AND LAT > $2 AND LAT < $3 AND LNG > $4 AND LNG < $5 "
				+ " GROUP BY LCODE, MEAN, TCOUNT, DONG, LAT, LNG, RDATE) AS LATEST_STATS"
				+ " INNER JOIN"
				+ " APT_STATS ON APT_STATS.LCODE = LATEST_STATS.LCODE AND APT_STATS.RDATE = LATEST_STATS.RDATE "
	...
	async.waterfall([
		...
		,function(callback) {
			...
			if (rdate == undefined){
				...
				+ " FROM (SELECT DISTINCT ON(LCODE)  LCODE, MEAN, TCOUNT, CITY AS DONG,  LAT, LNG, MAX(RDATE) AS RDATE "
				+ " FROM APT_STATS"
				+ " WHERE RDATE > now() - interval '3 months' and geolevel = 'C' AND RTYPE = $1 "
				+ " AND LAT > $2 AND LAT < $3 AND LNG > $4 AND LNG < $5 "
				+ " GROUP BY LCODE, MEAN, TCOUNT, DONG, LAT, LNG, RDATE) AS LATEST_STATS"
				+ " INNER JOIN"
				+ " APT_STATS ON APT_STATS.LCODE = LATEST_STATS.LCODE AND APT_STATS.RDATE = LATEST_STATS.RDATE "
